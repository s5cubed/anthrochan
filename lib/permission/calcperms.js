...
module.exports = (req, res) => {
	...
	if (req.session && res.locals && res.locals.user) {
		...
		if (res.locals.board != null) {
			...
			} else if (res.locals.board.trusted[user.username] != null) {
				// they are trusted user
				boardPermissions = new Permission();
				boardPermissions.setAll([
					Permissions.CREATE_ACCOUNT,
					Permissions.BYPASS_CAPTCHA,
					Permissions.BYPASS_FILE_APPROVAL,
					Permissions.USE_MARKDOWN_GENERAL,
				]);
				boardRolePermission = roleManager.roles.TRUSTED_USER;
			}
			if (boardPermissions) {
